...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      ...
      description:
        "Cognivis может работать с различными провайдерами LLM. Этот сервис будет обеспечивать обработку чата.",
    },
    userSetup: {
    ...
    survey: {
      title: "Добро пожаловать в Cognivis",
      description:
        "Помогите нам сделать Cognivis, созданным с учётом ваших потребностей (необязательно).",
      email: "Какой у вас адрес электронной почты?",
      useCase: "Для чего вы будете использовать Cognivis?",
      useCaseWork: "Для работы",
      useCasePersonal: "Для личного использования",
      useCaseOther: "Другое",
      comment: "Откуда вы узнали о Cognivis?",
      commentPlaceholder:
  ...
  agent: {
    ...
    settings: {
      ...
      "intelligent-skill-selection": {
        ...
        description:
          "Позволяет использовать неограниченное количество инструментов и сократить использование токенов до 80% на запрос – Cognivis автоматически выбирает наиболее подходящие навыки для каждого запроса.",
        "max-tools": {
  ...
  api: {
    title: "API ключи",
    description:
      "API ключи позволяют владельцу программно получать доступ к этому экземпляру Cognivis и управлять им.",
    ...
    modal: {
      ...
      helper:
        "После создания API-ключ можно использовать для программного доступа к этому экземпляру Cognivis и его настройки.",
      name: {
        label: "Название",
  ...
  llm: {
    ...
    description:
      "Это учетные данные и настройки для вашего предпочтительного поставщика чата и встраивания LLM. Важно, чтобы эти ключи были актуальными и правильными, иначе Cognivis не будет работать должным образом.",
  ...
  transcription: {
    ...
    "warn-start":
      "Использование локальной модели whisper на машинах с ограниченной оперативной памятью или процессором может привести к зависанию Cognivis при обработке медиафайлов.",
    "warn-recommend":
  ...
  embedding: {
    ...
    "desc-end":
      "Встраивание - это процесс превращения текста в векторы. Эти учетные данные необходимы для превращения ваших файлов и подсказок в формат, который Cognivis может использовать для обработки.",
    provider: {
  ...
  vector: {
    ...
    description:
      "Это учетные данные и настройки для того, как будет функционировать ваш экземпляр Cognivis. Важно, чтобы эти ключи были актуальными и правильными.",
    provider: {
  ...
  privacy: {
    ...
    description:
      "Это ваша конфигурация для того, как подключенные сторонние поставщики и Cognivis обрабатывают ваши данные.",
  ...
  connectors: {
    ...
    upload: {
      ...
      "privacy-notice":
        "Эти файлы будут загружены в процессор документов на этом экземпляре Cognivis. Файлы не отправляются и не передаются третьим лицам.",
    },
    pinning: {
      what_pinning: "Что такое закрепление документа?",
      pin_explained_block1:
        "Когда вы <b>закрепляете</b> документ в Cognivis, мы вставляем всё его содержимое в окно запроса, чтобы LLM мог полностью его понять.",
      pin_explained_block2:
        "Это работает лучше всего с <b>моделями с большим контекстом</b> или небольшими файлами, критичными для базы знаний.",
      pin_explained_block3:
        "Если по умолчанию ответы Cognivis вас не удовлетворяют, закрепление — отличный способ получить более качественные ответы одним кликом.",
      accept: "Хорошо, понял",
    },
    watching: {
      what_watching: "Что делает функция наблюдения за документом?",
      watch_explained_block1:
        "Когда вы <b>наблюдаете</b> за документом в Cognivis, мы <i>автоматически</i> синхронизируем его содержимое с оригинальным источником через регулярные интервалы. Это автоматически обновляет содержимое во всех рабочих пространствах, где используется этот файл.",
  ...
  chat_window: {
    ...
    memories: {
      ...
      modal: {
        ...
        label: "Память",
        placeholder:
          "Например, имя пользователя – Джо, он работает над Cognivis и т.д.",
        create: "Создать",
        save: "Сохранить",
        cancel: "Отменить",
      },
  ...
  customization: {
    interface: {
      ...
      description:
        "Настройте свои предпочтения пользовательского интерфейса для Cognivis.",
    },
    branding: {
      title: "Брендинг и создание продуктов с собственной меткой.",
      description:
        "Настройте свою версию Cognivis с использованием собственных брендинговых элементов.",
    },
    chat: {
      title: "Чат",
      description: "Настройте свои предпочтения для чата с Cognivis.",
      auto_submit: {
    ...
    items: {
      ...
      "display-language": {
        ...
        description:
          "Выберите предпочитаемый язык для отображения пользовательского интерфейса Cognivis – когда доступны переводы.",
      },
      logo: {
    ...
    publish: {
      ...
      generic: {
        unauthenticated: {
          ...
          description:
            "Для публикации материалов необходимо сначала пройти аутентификацию в сообществе Cognivis.",
  ...
  security: {
    ...
    password: {
      ...
      description:
        "Защитите ваш экземпляр Cognivis паролем. Если вы забудете его, метода восстановления не существует, поэтому убедитесь, что вы сохранили этот пароль.",
  ...
  telegram: {
    ...
    description:
      "Подключите свою инстанцию Cognivis к Telegram, чтобы вы могли общаться со своими рабочими пространствами с любого устройства.",
    setup: {
      step1: {

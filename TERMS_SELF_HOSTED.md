# Cognivis Self-Hosted: Data Privacy & Terms of Service

This document outlines the privacy standards, data handling procedures, and licensing terms for the self-hosted version of Cognivis, developed by Mintplex Labs Inc.

## 1. Data Sovereignty & Local-First Architecture
Cognivis is designed as a **local-first** application. When utilizing the self-hosted version (Docker, Desktop, or Source):
...
* **Air-Gap Capability:** Cognivis can be operated in a strictly air-gapped environment with no internet connectivity, provided local LLM and Vector database providers (e.g., Ollama, LocalAI, LanceDB) are utilized.

## 2. Telemetry and Analytics
To improve software performance and stability, Cognivis includes an optional telemetry feature.
...
Cognivis allows users to connect to external services (e.g., OpenAI, Anthropic, Pinecone). 
...
_by default, Cognivis does **everything on-device first** - so you would have to manually configure and enable these integrations to be subject to third party terms._
...
* **Environment Security:** The user is responsible for securing the host environment, including network firewalls, SSL/TLS encryption, and access control for the Cognivis instance.
...
* **License:** The Cognivis core is provided under the **MIT License**.

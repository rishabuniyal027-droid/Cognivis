...
Use the Dockerized version of Cognivis for a much faster and complete startup of Cognivis.

### Minimum Requirements

> [!TIP]
> Running Cognivis on AWS/GCP/Azure?
...
\*Cognivis by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*Cognivis by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized Cognivis!
...
> the docker container using Cognivis as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mintplexlabs/Cognivis
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run Cognivis in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/Cognivis && \
...
$env:STORAGE_LOCATION="$HOME\Documents\Cognivis"; `
...
services:
  Cognivis:
    image: mintplexlabs/Cognivis
    container_name: Cognivis
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
    ...
    volumes:
      - Cognivis_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using Cognivis! All your data and progress will persist between
...
- `touch server/storage/Cognivis.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of Cognivis easier.

### Use the Midori AI Subsystem to Manage Cognivis

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the Cognivis docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in Cognivis.
